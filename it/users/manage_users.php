...
<div class="card-body">
    <form action="" id="user-form">
        ...
        <div class="form-group">
            ...
            <text area = "username" id="" cols="30" rows="2" class="form-control form no-resize"><?php echo isset($username)? $username : ''; ?></textarea>
        </div>
        <div class="form-group">
            <label for="group_id" class="control-label">User Role</label>
            <select name="group_id" id="group_id" class="custom-select select2" required>
            <option value=""></option>
            <?php
             $qry = $conn->query("SELECT *FROM 'user_groups' order by group_name asc");
             while($row= $qry->fetch_assoc()):
            ?>
            <option value="<?php echo $row['id'] ?>"<?php echo isset($group_id) && $group_id == $row['id'] ? 'selected' : '' ?>><?php echo $row['group_name'] ?></option>
            <?php endwhile; ?>
             </select>
             </div>
