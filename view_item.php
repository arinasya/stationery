<?php 
 $items = $conn->query("SELECT * FROM `items`  where md5(id) = '{$_GET['id']}' ");
 if($items->num_rows > 0){
     foreach($items->fetch_assoc() as $k => $v){
         $$k= stripslashes($v);
     }
    ...
    $inventory = $conn->query("SELECT * FROM items where item_id = ".$id);
    $inv = array();
    ...
    <div class="container px-4 px-lg-5 my-5">
        ...
        <div class="row gx-4 gx-lg-5 align-items-center">
            ...
            <div class="col-md-6">
                ...
                <div class="fs-5 mb-5">
                RM<span id="price"><?php echo number_format($inv[0]['price']) ?></span>
                <br>
                
                <form action="" id="add-cart">
                <div class="d-flex">
                    <input type="hidden" name="price" value="<?php echo $inv[0]['price'] ?>">
                    <input type="hidden" name="item_id" value="<?php echo $inv[0]['id'] ?>">
...
<section class="py-5 bg-light">
    <div class="container px-4 px-lg-5 mt-5">
        ...
        <?php 
            $items = $conn->query("SELECT * FROM `items` where status = 1 and (vendor_id = '{$vendor_id}' or item_id = '{$item_id}') and id !='{$id}' order by rand() limit 4 ");
            while($row = $products->fetch_assoc()):
                ...
                $inventory = $conn->query("SELECT * FROM items where item_id = ".$row['id']);
                $_inv = array();
                foreach($row as $k=> $v){
