	...
	<div class="card-body">
		<form action="" id="product-form">
            ...
            <div class="form-group">
                ...
                <option value=""></option>
                <?php
                    $qry = $conn->query("SELECT * FROM `vendors` order by name asc");
                    while($row= $qry->fetch_assoc()):
                ?>
                <option value="<?php echo $row['id'] ?>" <?php echo isset($vendor_id) && $vendor_id == $row['id'] ? 'selected' : '' ?>><?php echo $row['vendor_name'] ?></option>
                <?php endwhile; ?>
                </select>
			</div>
			<div class="form-group">
	...
	$(document).ready(function(){
        ...
        $('#vendor_id').change(function(){
            ...
            Object.keys(items).map(k=>{
                if(k == cid){
                    Object.keys(items[k]).map(i=>{
                        if('<?php echo isset($item_id) ? $item_id : 0 ?>' == items[k][i].id){
                            opt += "<option value='"+items[k][i].id+"' selected>"+ items[k][i].item+"</option>";
                        }else{
                            opt += "<option value='"+items[k][i].id+"'>"+items[k][i].item+"</option>";
                        }
                    })
                }
            })
        ...
        if(parseInt("<?php echo isset($vendor_id) ? $vendor_id : 0 ?>") > 0){
            console.log('test')
            start_loader()
            setTimeout(() => {
