    ...
    <div class="container">
        ...
        <div class="card rounded-0">
            <div class="card-body">
                ...
                <?php 
                    $qry = $conn->query("SELECT c.*,i.name,i.price,i.id as iid from `cart` c inner join `items` i on i.id=c.item_id where c.client_id = ".$_settings->userdata('id'));
                    while($row= $qry->fetch_assoc()):
                        $upload_path = base_app.'/uploads/product_'.$row['iid'];
                        $img = "";
                        foreach($row as $k=> $v){
                        ...
                        if(is_dir($upload_path)){
                            ...
                            if(isset($fileO[2]))
                                $img = "uploads/product_".$row['iid']."/".$fileO[2];
                            // var_dump($fileO);
                        }
                ?>
