...
<div class="d-flex w-100 justify-content-between mb-2 py-2 border-bottom cart-item">
    <div class="d-flex align-items-center col-8">
        ...
        <div>
            ...
            <p class="mb-1 mb-sm-1"><small><b>Price:</b> RM<span class="price"><?php echo $item_price ?></span></small></p>
    ...
    <div class="col text-right align-items-center d-flex justify-content-end">
        <h4><b class="total-amount">RM<?php echo number_format((float)($item_price * $row['quantity']), $decimal_places) ?></b></h4>
    </div>
</div>
<?php endwhile; ?>
<div class="d-flex w-100 justify-content-between mb-2 py-2 border-bottom">
    ...
    <div class="col d-flex justify-content-end"><h4 id="grand-total">RM-</h4></div>
</div>
...
function calc_total() {
    ...
    $('.total-amount').each(function() {
        var amount = $(this).text().replace('RM', '').replace(/,/g, '').trim();
        amount = parseFloat(amount);
        total += amount;
    });
    var maxDecimalPlaces = getMaxDecimalPlaces();
    $('#grand-total').text('RM' + total.toLocaleString('en-US', { minimumFractionDigits: maxDecimalPlaces, maximumFractionDigits: maxDecimalPlaces }));
}
...
function qty_change(type, _this) {
    ...
    _this.closest('.cart-item').find('.total-amount').text('RM' + new_total.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
    calc_total();
